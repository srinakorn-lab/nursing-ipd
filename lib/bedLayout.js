...
export function getBedUnits(ward, layoutCfg) {
  ...
  // Explicit-room wards: real room codes given directly (single-bed corridor
  // wards whose numbering doesn't follow the generated 01..N sequence, e.g. 503A-517A)
  if (wardLayout.explicitRooms) {
    const units = wardLayout.explicitRooms.map(r => ({
      code: r.code, room: r.code, roomNum: r.code,
      isShared: false, isExtra: false, category: roomTypes[r.code] || r.category || null,
    }))
