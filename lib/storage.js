...
// Returns true only if every shift POST reached D1 successfully (2xx).
// Callers must check this — a failed sync used to be swallowed silently,
// leaving localStorage looking "saved" while D1 (and every other device) never got it.
...
// Returns true only if the D1 POST succeeded (2xx) — see note on saveEntries above.
export async function saveDailyEntry(year, month, wardId, day, shift, entry) {
  ...
  return fetch(`/api/daily/${year}/${month}/${wardId}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-device-id': getDeviceId() },
    body: JSON.stringify({ ...entry, day, shift: shift.toLowerCase() }),
  }).then(r => r.ok).catch(() => false)
}

// Bed availability — latest per ward
export async function apiLoadBeds() {
  try {
...
export async function saveBeds(entry) {
  if (typeof window === 'undefined') return
...
export async function apiLoadBedsHistory(ward, limit = 50) {
  try {
    const p = new URLSearchParams()
    if (ward) p.set('ward', ward)
    p.set('limit', String(limit))
    const res = await fetch(`/api/beds/history?${p}`)
    return res.ok ? res.json() : []
  } catch { return [] }
}

// ── Beds map (cinema-style bed status) ───────────────────────────
export async function apiLoadBedsMap(ward) {
  try {
    const q = ward ? `?ward=${encodeURIComponent(ward)}` : ''
    const res = await fetch(`/api/beds-map${q}`)
    return res.ok ? res.json() : {}
  } catch { return {} }
}

export async function apiSaveBed(bed) {
  try {
    const res = await fetch('/api/beds-map', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-device-id': getDeviceId() },
...
export async function apiMoveBed(fromWard, fromBed, toWard, toBed) {
  try {
    const res = await fetch('/api/beds-map/move', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-device-id': getDeviceId() },
      body: JSON.stringify({ fromWard, fromBed, toWard, toBed }),
    })
    return res.ok ? res.json() : { ok: false }
  } catch { return { ok: false } }
}

export async function apiClearBedsMapWard(ward) {
  try {
    await fetch(`/api/beds-map?ward=${encodeURIComponent(ward)}`, { method: 'DELETE' })
  } catch {}
}

// Delete: all / one ward / single record
export async function deleteAllBeds() {
  await fetch('/api/beds', { method: 'DELETE' }).catch(() => {})
}
export async function deleteBedsForWard(ward) {
  await fetch(`/api/beds?ward=${encodeURIComponent(ward)}`, { method: 'DELETE' }).catch(() => {})
}
export async function deleteBedRecord(id) {
  await fetch(`/api/beds?id=${id}`, { method: 'DELETE' }).catch(() => {})
}
